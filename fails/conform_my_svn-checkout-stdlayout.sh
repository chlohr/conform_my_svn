...
#	rename it './tags/[some name based on sub_tree_A + tag_name]/sub_tree_A/sub_tree_B'
# - for each path in the form './sub_tree_A/branches/branch_name/sub_tree_B' 
#	rename it './branches/[some name based on sub_tree_A + branch_name]/sub_tree_A/sub_tree_B'
...
while read LINE ; do
  ...
  if [ "$SRC" ]; then
    ...
    DST=${SRC/\/tags*/}
    set_tmp12
    ACTION="repocutter pathrename \"^$SRC\" \"tags/$TAG/$DST\" < $MYTMP1 > $MYTMP2"
    echo "echo [$n] \"$ACTION\""
    echo "$ACTION"
  fi
done < $TMP
...
while read LINE ; do
  ...
  if [ "$SRC" ]; then
    ...
    DST=${SRC/\/branches*/}
    set_tmp12
    ACTION="repocutter pathrename \"^$SRC\" \"branches/$BRANCH/$DST\" < $MYTMP1 > $MYTMP2"
    echo "echo [$n] \"$ACTION\""
    echo "$ACTION"
  fi
done < $TMP
...
echo "echo FINAL \"repocutter swapcheck < ${REPO}_new.svn\""
echo "repocutter swapcheck < ${REPO}_new.svn"
